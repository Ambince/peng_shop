...
import { fetchMilestones } from '@/api/gift/milestone';
...
const fields = ['id', 'content', 'keyword', 'time', 'status', 'operation'];

export function MilestoneTable({ type }: { type: string }): JSX.Element {
  ...
  const initData = useCallback(async () => {
    ...
    const { info, page, error } = await fetchMilestones(copyCondition);
    if (!info || error) {
  ...
  useEffect(() => {
    if (flushPageType === GiftCampaignType.MILESTONE)
              ...
              render={(_: any, record: CampaignInfo) => {
                return (
                  <GiftColumn
                    ...
                    type={GiftCampaignType.MILESTONE}
                  />
                );
              }}
              title={column.title}
