...
export function SuperCampaign(): JSX.Element {
  const { t } = useTranslation();
  const { flushPageType } = useSelector((store) => store.gift);
  ...
  const copyCondition = JSON.parse(JSON.stringify(initCondition));
  delete copyCondition.appId;
  ...
  useEffect(() => {
    ...
    dispatch(setCurrentGiftType({ type: GiftCampaignType.ALL }));
  }, [condition]);

  useEffect(() => {
    if (flushPageType === GiftCampaignType.ALL) {
